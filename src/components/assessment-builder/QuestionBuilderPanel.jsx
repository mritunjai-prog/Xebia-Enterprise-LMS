...
export const QuestionBuilderPanel = ({
  ...
  const handleGenerateQuestions = async () => {
    ...
    if (remainingToGenerate === 0) {
      toast.add(
        `You already have ${questions.length} questions. Remove some to generate more.`,
        "info",
      );
      return;
    }
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        toast.add(
          "Failed to parse Excel file. Please ensure it follows the template format.",
          "error",
        );
      } finally {
        setIsGeneratingAi(false);
  ...
  const handleExportQuestions = () => {
    ...
    XLSX.writeFile(
      wb,
      `${config?.title?.replace(/[^a-z0-9]/gi, "_") || "Assessment"}_Questions.xlsx`,
    );
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto justify-start xl:justify-end">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center justify-center p-2 rounded-lg text-white disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all duration-300 ${isGeneratingAi ? "bg-primary-glow scale-110" : "bg-[#6C1D5F] hover:bg-primary-glow hover:-translate-y-0.5 hover:shadow-lg hover:shadow-fuchsia-900/30"}`}
              title="Generate with AI"
            >
              <Wand2
                className={`w-4 h-4 ${isGeneratingAi ? "animate-bounce text-primary-glow" : ""}`}
              />
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col bg-neutral-100/50 dark:bg-[#050505] relative">
        {!isConfigComplete && (
          <div className="absolute inset-0 bg-white/60 dark:bg-black/60 backdrop-blur-[2px] z-20 flex items-center justify-center">
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-6 rounded-2xl shadow-xl max-w-sm text-center space-y-3">
              <div className="w-12 h-12 bg-destructive/10 dark:bg-destructive text-destructive dark:text-destructive rounded-full flex items-center justify-center mx-auto mb-2">
                <Library className="w-6 h-6" />
              </div>
              <h3 className="font-bold text-neutral-900 dark:text-white">Configuration Required</h3>
              <p className="text-sm text-neutral-500 dark:text-neutral-400">
                Please complete all mandatory fields in the Configuration panel (Title, Topic,
                Course, Type, Difficulty, Duration, Marks) before adding questions.
              </p>
            </div>
          </div>
        )}
        <div className="w-full max-w-4xl mx-auto space-y-4 pb-32">
          {questions.length === 0 && !addingManual && (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-sm text-neutral-500 mt-1 max-w-xs">
                Add questions using the buttons below to see them appear here instantly.
              </p>
            </div>
          )}
          ...
          {questions.map((q, idx) => (
            <div
              key={q.id || idx}
              className="space-y-2 group relative border-b border-neutral-100 dark:border-neutral-800 pb-4"
            >
              ...
              <div className="absolute -right-4 top-0 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 translate-x-full z-10">
                <button
                  ...
                  className="p-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-sm text-neutral-500 hover:text-[#6C1D5F]"
                >
                  <Edit className="w-4 h-4" />
                </button>
                <button
                  onClick={() => handleDelete(idx)}
                  className="p-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-sm text-neutral-500 hover:text-destructive"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    onClick={() => handleEdit(idx)}
                    className="p-1.5 text-neutral-400 hover:text-accent-2 bg-neutral-100 dark:bg-neutral-800 hover:bg-accent-2/10 dark:hover:bg-accent-2 rounded-lg transition-colors cursor-pointer"
                  >
                    <Edit className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleDelete(idx)}
                    className="p-1.5 text-neutral-400 hover:text-destructive bg-neutral-100 dark:bg-neutral-800 hover:bg-destructive/10 dark:hover:bg-destructive rounded-lg transition-colors cursor-pointer"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="space-y-1 pl-6">
                  {q.options.map((opt, oIdx) => {
                    ...
                    return (
                      <div
                        key={oIdx}
                        className={`flex items-center gap-2 p-1.5 rounded-lg border transition-colors cursor-pointer text-sm ${isCorrect ? "border-accent-2 bg-accent-2/10 dark:bg-accent-2 text-accent-2 dark:text-accent-2 ring-1 ring-accent-2" : "border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50 hover:bg-neutral-100 dark:hover:bg-neutral-900 text-neutral-700 dark:text-neutral-300"}`}
                      >
                        <input
                          type="radio"
                          disabled
                        ...
                        {isCorrect && <CheckCircle className="w-4 h-4 text-accent-2 shrink-0" />}
                      </div>
                    );
                  })}
                </div>
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          disabled
                          checked={isCorrect}
                          className="w-4 h-4 text-accent-2 rounded"
                        />
                        <span className="flex-1">{opt}</span>
                        {isCorrect && <CheckCircle className="w-4 h-4 text-accent-2 shrink-0" />}
                      </div>
              ...
              {q.type === "short_answer" && (
                <div className="pl-6">
                  <input
                    ...
                    className="w-full h-12 px-4 bg-neutral-50 dark:bg-neutral-900/50 rounded-xl border border-neutral-200 dark:border-neutral-800 text-sm"
                  />
                </div>
              )}
              ...
              {q.type === "paragraph" && (
                <div className="pl-6">
                  <textarea
                    disabled
                    placeholder="Long paragraph response..."
                    className="w-full h-32 p-4 bg-neutral-50 dark:bg-neutral-900/50 rounded-xl border border-neutral-200 dark:border-neutral-800 text-sm resize-none"
                  />
                </div>
              )}
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-1.5 text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg"
                    >
                    ...
                    <div className="border border-neutral-200 dark:border-neutral-800 rounded-xl overflow-hidden">
                      <div className="flex gap-1 p-2 bg-neutral-50 dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800">
                        {["B", "I", "U", "{}", "∑"].map((tool, i) => (
                          <button
                            key={i}
                            className="w-6 h-6 flex items-center justify-center text-xs font-bold text-neutral-600 hover:bg-neutral-200 dark:hover:bg-neutral-800 rounded"
                          >
                            {tool}
                          </button>
                        ))}
                      ...
                      <textarea
                        ...
                        rows={3}
                        className="w-full p-3 text-sm focus:outline-none dark:bg-neutral-900"
                    ...
                    <div className="space-y-2">
                      ...
                      {draftOptions.map((opt, i) => (
                        <div key={i} className="flex items-center gap-3">
                          <input
                            type={config?.type === "multiple_select" ? "checkbox" : "radio"}
                            disabled
                          ...
                          <input
                            ...
                            className="flex-1 px-3 py-2 bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl text-sm"
                          />
                        </div>
                      ))}
                      <button
                        onClick={() => setDraftOptions([...draftOptions, ""])}
                        className="text-xs font-bold text-[#6C1D5F] mt-2"
                      >
                        + Add Option
                      </button>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-neutral-100 dark:border-neutral-800 mt-6">
                    <div className="flex gap-3">
                      <label className="text-xs font-bold text-neutral-500 flex items-center gap-1.5">
                        ...
                        <input
                          type="number"
                          defaultValue={config.marks || 1}
                          className="w-16 px-2 py-1 bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-lg"
                        />
                      </label>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => setAddingManual(false)}
                        className="px-4 py-2 text-xs font-bold text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl"
                      >
                        Cancel
                      </button>
                      <button
                        onClick={handleSaveManual}
                        className="px-4 py-2 bg-[#6C1D5F] hover:bg-[#84117C] text-white text-xs font-bold rounded-xl shadow-md"
                      >
                        Save Question
                      </button>
                    </div>
