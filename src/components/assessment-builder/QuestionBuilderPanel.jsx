      ...
      <div className="px-4 xl:px-6 py-4 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 border-b border-neutral-200 dark:border-neutral-800 bg-white/50 dark:bg-neutral-900/50 backdrop-blur shrink-0 z-10">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto justify-start xl:justify-end">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <Wand2 className={`w-4 h-4 ${isGeneratingAi ? 'animate-bounce text-primary-glow' : ''}`} />
            </button>
          </div>
        </div>
      </div>
            ...
            {questions.map((q, idx) => (
              <div key={q.id || idx} className="space-y-2 group relative border-b border-neutral-100 dark:border-neutral-800 pb-4">
                ...
                <div className="absolute -right-4 top-0 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 translate-x-full z-10">
                   ...
                   <button onClick={() => handleDelete(idx)} className="p-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-sm text-neutral-500 hover:text-destructive"><Trash2 className="w-4 h-4" /></button>
                </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => handleEdit(idx)} className="p-1.5 text-neutral-400 hover:text-accent-2 bg-neutral-100 dark:bg-neutral-800 hover:bg-accent-2/10 dark:hover:bg-accent-2 rounded-lg transition-colors cursor-pointer"><Edit className="w-4 h-4" /></button>
                    <button onClick={() => handleDelete(idx)} className="p-1.5 text-neutral-400 hover:text-destructive bg-neutral-100 dark:bg-neutral-800 hover:bg-destructive/10 dark:hover:bg-destructive rounded-lg transition-colors cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                ...
                {(q.type === 'mcq' || q.type === 'true_false') && q.options && (
                  <div className="space-y-1 pl-6">
                    {q.options.map((opt, oIdx) => {
                      ...
                      return (
                        <div key={oIdx} className={`flex items-center gap-2 p-1.5 rounded-lg border transition-colors cursor-pointer text-sm ${isCorrect ? 'border-accent-2 bg-accent-2/10 dark:bg-accent-2 text-accent-2 dark:text-accent-2 ring-1 ring-accent-2' : 'border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50 hover:bg-neutral-100 dark:hover:bg-neutral-900 text-neutral-700 dark:text-neutral-300'}`}>
                          <input type="radio" disabled checked={isCorrect} className="w-4 h-4 text-accent-2" />
                ...
                {q.type === 'multiple_select' && q.options && (
                  <div className="space-y-1 pl-6">
                    {q.options.map((opt, oIdx) => {
                      ...
                      return (
                        <div key={oIdx} className={`flex items-center gap-2 p-1.5 rounded-lg border transition-colors cursor-pointer text-sm ${isCorrect ? 'border-accent-2 bg-accent-2/10 dark:bg-accent-2 text-accent-2 dark:text-accent-2 ring-1 ring-accent-2' : 'border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50 hover:bg-neutral-100 dark:hover:bg-neutral-900 text-neutral-700 dark:text-neutral-300'}`}>
                          <input type="checkbox" disabled checked={isCorrect} className="w-4 h-4 text-accent-2 rounded" />
                          <span className="flex-1">{opt}</span>
                          {isCorrect && <CheckCircle className="w-4 h-4 text-accent-2 shrink-0" />}
                        </div>
                      );
