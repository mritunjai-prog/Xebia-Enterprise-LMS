...
import { Bell, Search, Menu, Settings, Sun, Moon, X, BookOpen, Tag as TagIcon, CheckCircle, Code, MessageSquare, User, Layers, ClipboardCheck } from "lucide-react";
import { studentProfile, notifications, batchInfo, upcomingAssessments } from "@/features/student/mocks/dummy-data";
...
export function StudentNavbar({ isMobileOpen, setIsMobileOpen }) {
  ...
  const [searchResults, setSearchResults] = useState({ features: [], courses: [], batches: [], assessments: [] });
  ...
  const MOCK_BATCHES = [
    { id: 'b1', name: batchInfo.batchName, type: batchInfo.trainer }
  ];
    ...
    if (searchQuery.trim().length >= 1) {
      CourseService.getCourses()
        .then((courses) => {
          ...
          setSearchResults({
            features: STUDENT_FEATURES.filter(f => f.title.toLowerCase().includes(lower)).slice(0, 3),
            ...
            assessments: MOCK_ASSESSMENTS.filter(a => a.title.toLowerCase().includes(lower)).slice(0, 3)
          });
        });
    } else {
  ...
  const initials = tempProfile?.name
    ? tempProfile.name.split(" ").map((n) => n[0]).join("").toUpperCase()
    : "S";
      ...
      <div className="header-search" ref={searchRef}>
        ...
        {isSearchOpen && searchQuery.trim().length >= 1 && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-xl shadow-xl overflow-hidden z-50 py-2 custom-scrollbar" style={{ maxHeight: '400px', overflowY: 'auto' }}>
            {searchResults.features.length === 0 && searchResults.courses.length === 0 && searchResults.batches.length === 0 && searchResults.assessments.length === 0 ? (
              ...
              <>
                {searchResults.features.length > 0 && (
                  <div className="mb-2">
                    <div className="px-4 py-1.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Features & Pages</div>
                    {searchResults.features.map(feat => (
                      ...
                      >
                        ...
                        <span className="text-sm font-semibold text-gray-900 dark:text-white truncate">{feat.title}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {searchResults.batches.length > 0 && (
                  <div className="mb-2">
                    <div className="px-4 py-1.5 text-xs font-bold text-gray-400 uppercase tracking-wider">My Batches</div>
                    {searchResults.batches.map(batch => (
                      ...
                      >
                        ...
                        <div className="flex flex-col truncate">
                          <span className="text-sm font-semibold text-gray-900 dark:text-white truncate">{batch.name}</span>
                ...
                {searchResults.assessments.length > 0 && (
                  <div className="mb-2">
                    <div className="px-4 py-1.5 text-xs font-bold text-gray-400 uppercase tracking-wider">Assessments</div>
                    {searchResults.assessments.map(assessment => (
                      ...
                      >
                        ...
                        <div className="flex flex-col truncate">
                          <span className="text-sm font-semibold text-gray-900 dark:text-white truncate">{assessment.title}</span>
                          <span className="text-xs text-gray-500 truncate">{assessment.status}</span>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="header-actions">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-80 bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] shadow-xl rounded-xl p-0 overflow-hidden">
              <div className="px-4 py-3 font-bold text-sm text-gray-900 dark:text-white border-b border-gray-100 dark:border-[#2e2e3e]">Notifications</div>
              {notifications.slice(0, 3).map((notification) => (
                ...
                >
                  <div className="flex w-full items-center justify-between gap-2">
                    <span className={`font-semibold text-sm ${!notification.read ? 'text-[#6C1D5F] dark:text-[#D3CCEC]' : 'text-gray-800 dark:text-gray-200'}`}>
                      {notification.title}
                    </span>
                    <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0">{notification.timestamp}</span>
                  </div>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="text-white text-xs font-semibold text-center leading-tight px-2">Change Image</span>
                  </div>
                </div>
