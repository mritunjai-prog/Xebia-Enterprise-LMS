  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <table className="w-full text-sm text-center">
              <thead>
                <tr>
                  ...
                  {Object.keys(heatmapData[0]).filter(k => k !== 'region').map(k => (
                    <th key={k} className="p-2 text-gray-500">{k}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {heatmapData.map((row, i) => (
                  <tr key={i}>
                    ...
                    {Object.entries(row).filter(([k]) => k !== 'region').map(([k, v], j) => (
        ...
        <div className="bg-white dark:bg-[#111111] p-6 rounded-2xl border border-gray-200 dark:border-white/10 hover:border-[#6C1D5F] dark:hover:border-[#FFACE8] transition-colors duration-300">
          <h3 className="font-bold text-gray-900 dark:text-white mb-6">Region-wise Coverage Chart</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={regionCoverageData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="coverage" fill="#FF6200" radius={[4, 4, 0, 0]} barSize={40} style={{ filter: 'url(#shadow3d)' }} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white dark:bg-[#111111] p-6 rounded-2xl border border-gray-200 dark:border-white/10 hover:border-[#6C1D5F] dark:hover:border-[#FFACE8] transition-colors duration-300">
          <h3 className="font-bold text-gray-900 dark:text-white mb-6">Project-wise Participation Chart</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={projectData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }} barSize={20}>
                ...
                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{fill: '#888', fontWeight: 600}} />
        ...
        <div className="bg-white dark:bg-[#111111] p-6 rounded-2xl border border-gray-200 dark:border-white/10 hover:border-[#6C1D5F] dark:hover:border-[#FFACE8] transition-colors duration-300">
          <h3 className="font-bold text-gray-900 dark:text-white mb-6">Quarterly Participation Trend</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <Legend iconType="circle" />
                <Area type="monotone" dataKey="Coverage" stroke="#6C1D5F" fillOpacity={1} fill="url(#colorCov)" strokeWidth={3} style={{ filter: 'url(#shadow3d)' }} />
                <Area type="monotone" dataKey="Participation" stroke="#01AC9F" fillOpacity={1} fill="url(#colorPart)" strokeWidth={3} style={{ filter: 'url(#shadow3d)' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
