...
export const DateTimePicker = ({ type = "date", value, onChange }) => {
    ...
    for (let i = 1; i <= totalItems; i++) {
      ...
      items.push(
        <button
          ...
          className={`absolute w-8 h-8 -ml-4 -mt-4 rounded-full flex items-center justify-center text-xs font-bold transition-all ${isSelected ? "bg-[#6C1D5F] text-white shadow-md" : "text-neutral-700 dark:text-neutral-300 hover:bg-primary/10 dark:hover:bg-primary"}`}
          style={{ left: x, top: y }}
        >
          {displayVal}
        </button>,
  ...
  return (
    <div className="relative">
      ...
      >
        {type === "date" ? (
          <CalendarIcon className="w-4 h-4 text-[#6C1D5F]" />
        ) : (
          <Clock className="w-4 h-4 text-[#6C1D5F]" />
        )}
            ...
            >
              ...
              {type === "date" ? (
                <div>
                  ...
                  <div className="grid grid-cols-7 gap-1 text-center mb-2">
                    {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
                      <div key={d} className="text-[10px] font-bold text-neutral-400">
                        {d}
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex flex-col items-center">
                  <div className="flex gap-2 text-2xl font-black font-mono text-[#6C1D5F] dark:text-primary mb-2">
                    <button
                      onClick={() => setClockMode("hours")}
                      className={`hover:opacity-80 transition-opacity ${clockMode === "hours" ? "underline decoration-2" : ""}`}
                    >
                    ...
                    <button
                      ...
                      className={`hover:opacity-80 transition-opacity ${clockMode === "minutes" ? "underline decoration-2" : ""}`}
                    >
                    ...
                    <div className="flex flex-col text-[10px] ml-2 bg-neutral-100 dark:bg-neutral-800 rounded-lg overflow-hidden">
                      <button
                        ...
                        className={`px-2 py-1 ${period === "AM" ? "bg-[#6C1D5F] text-white" : "text-neutral-500"}`}
                      >
                        AM
                      </button>
                      <button
                        onClick={() => setPeriod("PM")}
                        className={`px-2 py-1 border-t border-neutral-200 dark:border-neutral-700 ${period === "PM" ? "bg-[#6C1D5F] text-white" : "text-neutral-500"}`}
                      >
                        PM
                      </button>
                    </div>
