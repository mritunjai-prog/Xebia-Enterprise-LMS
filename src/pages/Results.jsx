      ...
      <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-800 rounded-xl p-4 md:p-5 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="space-y-3.5 text-center md:text-left">
          <div className="flex flex-wrap gap-2 items-center justify-center md:justify-start">
            <span className="px-2.5 py-0.5 bg-[#6C1D5F]/10 text-[#6C1D5F] dark:text-primary font-bold rounded-lg text-[10px] uppercase font-mono">
              Exam Submission Closed
            </span>
            {submission.isEvaluated ?
            isPassed ?
            <span className="px-2.5 py-0.5 bg-accent-2/10 text-[#01AC9F] dark:bg-accent-2 dark:text-accent-2 font-bold rounded-lg text-[10px] uppercase font-mono">
                  Passing Outcome
                </span> :
            ...
            <span className="px-2.5 py-0.5 bg-destructive/10 text-destructive dark:bg-destructive dark:text-destructive font-bold rounded-lg text-[10px] uppercase font-mono animate-pulse">
        ...
        <div className="flex flex-col items-center shrink-0">
          <div className={`w-24 h-24 rounded-full border-4 flex flex-col items-center justify-center shadow-md transition-colors ${submission.isEvaluated ? isPassed ? 'border-accent-2 bg-accent-2/10 dark:bg-accent-2' : 'border-rose-500 bg-rose-50/20 dark:bg-rose-950/20' : 'border-destructive bg-destructive/10 dark:bg-destructive'}`}>
          ...
          {assessment.questions.map((q, idx) => {
            ...
            if (q.type === 'coding') {
              ...
              return (
                <div
                  key={q.id}
                  className={`bg-white dark:bg-neutral-900 border rounded-xl p-3 space-y-3 shadow-sm ${isEvaluated ? isPassed ? 'border-accent-2/20 dark:border-accent-2' : 'border-rose-200/50 dark:border-rose-950/50' : 'border-brand-border dark:border-neutral-700 dark:border-neutral-800'}`}>
                  ...
                  <div className="flex justify-between items-center border-b border-neutral-100 dark:border-neutral-800/60 pb-1.5">
                    ...
                    <span className="font-mono font-bold text-xs text-neutral-600 dark:text-neutral-300">
                      Awarded: <span className="text-[#6C1D5F] dark:text-primary">{codingAns?.score || marksAwarded}</span> / {q.marks} pts
                    </span>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center text-[10px] font-mono">
                      ...
                      <div className="p-2 bg-neutral-50 dark:bg-neutral-950 rounded-lg border border-neutral-100 dark:border-neutral-800/40">
                        ...
                        <span className={`font-bold ${isPassed ? 'text-accent-2' : 'text-destructive'}`}>{codingAns?.status || 'Submitted'}</span>
                      </div>
                      <div className="p-2 bg-neutral-50 dark:bg-neutral-950 rounded-lg border border-neutral-100 dark:border-neutral-800/40">
            ...
            return (
              <div
                key={q.id}
                className={`bg-white dark:bg-neutral-900 border rounded-xl p-3 space-y-2 shadow-sm ${isEvaluated ? isCorrect ? 'border-accent-2/20 dark:border-accent-2' : 'border-rose-200/50 dark:border-rose-950/50' : 'border-brand-border dark:border-neutral-700 dark:border-neutral-800'}`}>
                ...
                <div className="flex justify-between items-center border-b border-neutral-100 dark:border-neutral-800/60 pb-1.5">
                  ...
                  <span className="font-mono font-bold text-xs text-neutral-600 dark:text-neutral-300">
                    Awarded: <span className="text-[#6C1D5F] dark:text-primary">{marksAwarded}</span> / {q.marks} pts
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-2 bg-neutral-50 dark:bg-neutral-950 rounded-xl space-y-1.5 border border-neutral-100 dark:border-neutral-800/40">
                    <p className="text-[10px] text-neutral-500 dark:text-neutral-400 font-bold uppercase tracking-wide flex items-center gap-1">
                      ...
                      {isEvaluated && (
                      isCorrect ?
                      <CheckCircle className="w-3.5 h-3.5 text-accent-2 shrink-0" /> :
                  ...
                  {isEvaluated && (
                    <div className="p-2 bg-accent-2/5 dark:bg-accent-2 rounded-xl space-y-1.5 border border-accent-2/20">
                      <p className="text-[10px] text-accent-2 dark:text-accent-2 font-bold uppercase tracking-wide">Correct Template Answer</p>
                ...
                <div className="pt-3 border-t border-dashed border-neutral-100 dark:border-neutral-800 space-y-1.5">
                    {ansObj?.remarks &&
                  <p className="text-[11px] text-[#6C1D5F] dark:text-primary font-medium flex items-start gap-1">
