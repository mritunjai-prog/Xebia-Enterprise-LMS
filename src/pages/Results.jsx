    ...
    <div className="w-full max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-800 rounded-xl p-4 md:p-5 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="space-y-3.5 text-center md:text-left">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400">
              Student Examinee: {student?.name || "Academic Enrollee"}
            </p>
          </div>
        ...
        <div className="flex flex-col items-center shrink-0">
          <div
            className={`w-24 h-24 rounded-full border-4 flex flex-col items-center justify-center shadow-md transition-colors ${submission.isEvaluated ? (isPassed ? "border-emerald-500 bg-emerald-50/20 dark:bg-emerald-950/20" : "border-rose-500 bg-rose-50/20 dark:bg-rose-950/20") : "border-amber-500 bg-amber-50/20 dark:bg-amber-950/10"}`}
          >
            <span className="text-[9px] text-neutral-500 dark:text-neutral-400 font-bold uppercase tracking-wider">
              Your Score
            </span>
        ...
        <div className="space-y-4">
          {assessment.questions.map((q, idx) => {
            ...
            if (q.type === "coding") {
              ...
              const isPassed =
                ansObj?.isCorrect ||
                codingAns?.status === "Accepted" ||
                codingAns?.status === "Partially Accepted";
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-center text-[10px] font-mono">
                      <div className="p-2 bg-neutral-50 dark:bg-neutral-950 rounded-lg border border-neutral-100 dark:border-neutral-800/40">
                        <span className="text-[9px] text-neutral-500 dark:text-neutral-400 block font-sans">
                          LANGUAGE USED
                        </span>
                        <span className="text-neutral-800 dark:text-white font-bold">
                          {codingAns?.language?.toUpperCase() || "JAVASCRIPT"}
                        </span>
                      </div>
                      <div className="p-2 bg-neutral-50 dark:bg-neutral-950 rounded-lg border border-neutral-100 dark:border-neutral-800/40">
                        <span className="text-[9px] text-neutral-500 dark:text-neutral-400 block font-sans">
                        ...
                        <span
                          className={`font-bold ${isPassed ? "text-green-500" : "text-amber-500"}`}
                        >
                          {codingAns?.status || "Submitted"}
                        </span>
                      </div>
                      <div className="p-2 bg-neutral-50 dark:bg-neutral-950 rounded-lg border border-neutral-100 dark:border-neutral-800/40">
                        <span className="text-[9px] text-neutral-500 dark:text-neutral-400 block font-sans">
                      ...
                      <div className="p-2 bg-neutral-50 dark:bg-neutral-950 rounded-lg border border-neutral-100 dark:border-neutral-800/40">
                        ...
                        <span className="text-neutral-500 font-bold text-[10px]">
                          {codingAns?.submittedAt
                            ? new Date(codingAns.submittedAt).toLocaleTimeString()
                            : "N/A"}
                        </span>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {q.type === "file_upload" && studentAns && typeof studentAns === "object" ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="truncate">
                            {studentAns.name} ({studentAns.size})
                          </span>
                        </div>
                  ...
                  {isEvaluated && (
                    <div className="p-2 bg-emerald-500/5 dark:bg-emerald-950/10 rounded-xl space-y-1.5 border border-emerald-100/40">
                      <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wide">
                      ...
                      <div className="text-xs font-bold text-neutral-700 dark:text-neutral-300">
                        {q.type === "file_upload" ? (
                          <span className="text-neutral-500 dark:text-neutral-400 font-medium">
                            Trainer manual validation required
                          </span>
                        ) : q.type === "short_answer" || q.type === "paragraph" ? (
                          <span className="text-neutral-500 dark:text-neutral-400 font-medium">
                            Manual semantic validation required
                          </span>
                          ...
                          <ul className="list-disc pl-4 space-y-1 text-[#01AC9F]">
                            {q.correctAnswer.map((idxStr, optI) => (
                              <li key={optI}>
                                {q.options?.[Number(idxStr)] || `Option ${idxStr}`}
                              </li>
                            ))}
                          </ul>
                        ) : q.type === "mcq" || q.type === "true_false" ? (
                          <span className="text-[#01AC9F]">
