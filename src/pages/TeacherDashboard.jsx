  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total Students', value: totalStudents, icon: Users, color: '#6C1D5F', bg: 'bg-[#6C1D5F]/10 dark:bg-primary' },
          { label: 'Total Batches', value: totalBatches, icon: Layers, color: '#01AC9F', bg: 'bg-[#01AC9F]/10 dark:bg-accent-2' },
          { label: 'Total Assessments', value: totalAssessments, icon: FileSpreadsheet, color: '#F59E0B', bg: 'bg-destructive/10 dark:bg-destructive' },
            ...
            {batchPerformanceData.map((batch) => (
              <div key={batch.name} className="space-y-2 group cursor-default">
                <div className="flex items-center justify-between text-sm font-bold">
                  <span className="text-neutral-800 dark:text-neutral-200 group-hover:text-[#6C1D5F] dark:group-hover:text-primary transition-colors">{batch.name}</span>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm font-black text-[#6C1D5F] dark:text-primary font-mono w-10 text-right">{batch.avgScore}%</span>
                </div>
              </div>
            ))}
          </div>
