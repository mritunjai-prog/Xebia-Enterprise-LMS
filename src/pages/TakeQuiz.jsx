...
export const TakeQuiz = () => {
  ...
  const executeFinalSubmission = () => {
    ...
    navigate(`/student/results/${slug}/${completed.id}`);
  };
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 items-start w-full">
        ...
        <div className="flex-1 w-full bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-800 rounded-3xl p-4 md:p-6 space-y-4 shadow-sm min-h-[450px] flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-between items-center border-b border-brand-border/20 dark:border-neutral-800 pb-4">
              <span className="px-3 py-1.5 bg-[#6C1D5F]/10 dark:bg-primary text-[#6C1D5F] dark:text-primary font-bold rounded-lg text-sm font-mono">
                QUESTION #{currentQIndex + 1} • {currentQ.marks} PTS
              </span>
              {currentQ.required &&
              <span className="text-xs bg-destructive/10 text-destructive dark:text-destructive font-bold px-3 py-1 rounded-md uppercase font-mono">
                  Mandatory Question
                </span>
              }
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="space-y-3">
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 border rounded-2xl cursor-pointer flex items-center justify-between text-sm font-semibold transition-all ${isChecked ? 'bg-primary/10 border-[#6C1D5F] text-[#6C1D5F] dark:bg-primary dark:border-primary dark:text-primary shadow-sm' : 'border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800 text-neutral-700 dark:text-neutral-300'}`}>
                      
                        <span>{opt}</span>
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isChecked ? 'border-[#6C1D5F] dark:border-primary text-[#6C1D5F]' : 'border-neutral-300 dark:border-neutral-700'}`}>
                          {isChecked && <div className="w-2.5 h-2.5 rounded-full bg-[#6C1D5F] dark:bg-primary" />}
                        </div>
                      </div>);
              ...
              <div className="space-y-3">
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 border rounded-2xl cursor-pointer flex items-center justify-between text-sm font-semibold transition-all ${isChecked ? 'bg-accent-2/10 border-[#01AC9F] text-[#01AC9F] dark:bg-accent-2 dark:border-accent-2 dark:text-accent-2 shadow-sm' : 'border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800 text-neutral-700 dark:text-neutral-300'}`}>
                      
                        <span>{opt}</span>
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${isChecked ? 'bg-[#01AC9F] border-[#01AC9F] dark:bg-accent-2 dark:border-accent-2 text-white' : 'border-neutral-300 dark:border-neutral-700'}`}>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3.5 bg-accent-2/10 dark:bg-accent-2 rounded-2xl border border-accent-2/20">
                      <div className="flex items-center gap-2 text-xs truncate">
                        <FileText className="w-5 h-5 text-accent-2 shrink-0" />
          ...
          <div className="flex items-center justify-between pt-6 border-t border-brand-border/20 dark:border-neutral-800 mt-8">
            <div className="flex gap-3">
              <button
                ...
                className={`py-2 px-4 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all cursor-pointer ${flaggedQIds.includes(currentQ.id) ? 'bg-destructive text-white' : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'}`}>
        ...
        <div className="lg:w-80 shrink-0 bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-800 rounded-3xl p-6 shadow-sm min-h-[450px]">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-5 gap-2.5">
            {assessment.questions.map((q, idx) => {
              ...
              if (isCurrent) {
                ...
                bgStyle = 'bg-destructive text-white border-transparent shadow-amber-500/10';
              } else if (hasAnswered) {
                bgStyle = 'bg-accent-2 text-white border-transparent';
              }
          ...
          <div className="border-t border-brand-border/80 dark:border-neutral-700/80 pt-3.5 space-y-2 text-[10px] font-semibold text-neutral-500">
            ...
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-md bg-accent-2 shrink-0"></span>
              <span>Saved & Confirmed Response</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-md bg-destructive shrink-0"></span>
