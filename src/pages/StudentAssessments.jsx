      ...
      <div className="bg-white dark:bg-neutral-900 p-4 rounded-2xl shadow-sm border border-neutral-200 dark:border-neutral-800 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-800 mt-2">
          ...
          {(searchQuery || statusFilter !== 'All' || typeFilter !== 'All' || difficultyFilter !== 'All' || sortBy !== 'Newest') && (
            <button onClick={handleClearFilters} className="text-xs font-bold text-[#6C1D5F] dark:text-primary hover:underline cursor-pointer flex items-center gap-1 bg-[#6C1D5F]/10 dark:bg-primary/10 px-2 py-1 rounded-md">
      ...
      {filteredAssessments.length === 0 ? (
            ...
            <div className="overflow-x-auto w-full"><table className="w-full text-left border-collapse text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60 bg-white dark:bg-neutral-900">
                {filteredAssessments.map((as) => {
                  const statusColor = {
                    Active: 'bg-accent-2/10 text-accent-2 dark:bg-accent-2 dark:text-accent-2 border-accent-2/20 dark:border-accent-2',
                    Upcoming: 'bg-destructive/10 text-destructive dark:bg-destructive dark:text-destructive border-destructive/20 dark:border-destructive',
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold border ${as.difficulty === 'Easy' ? 'bg-accent-2/10 text-accent-2 border-accent-2/20 dark:bg-accent-2 dark:text-accent-2 dark:border-accent-2' : as.difficulty === 'Medium' ? 'bg-destructive/10 text-destructive border-destructive/20 dark:bg-destructive dark:text-destructive dark:border-destructive' : 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800'}`}>
                          {as.difficulty}
                        </span>
                      </td>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
          {filteredAssessments.map((as) => {
            ...
            }[as.difficulty] || 'text-accent-2 dark:text-accent-2 bg-accent-2/10 dark:bg-accent-2 border-accent-2/20 dark:border-accent-2';
            
            const statusColor = as.computedStatus === 'Active' 
              ? 'text-accent-2 bg-accent-2/10 border-accent-2/20 dark:bg-accent-2 dark:text-accent-2 dark:border-accent-2'
              : as.computedStatus === 'Completed'
                ? 'text-accent-2 bg-accent-2/10 border-accent-2/20 dark:bg-accent-2 dark:text-accent-2 dark:border-accent-2'
