...
export const StudentDashboard = () => {
  ...
  const handleStartAttempt = (aId) => {
    ...
    if (as.type === 'coding') {
      navigate(`/student/take-coding/${slug}`);
    } else {
      navigate(`/student/take/${slug}`);
    }
  };
  ...
  const handleViewResult = (sId) => {
    ...
    navigate(`/student/results/${slug}/${sId}`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#4A1E47] via-[#6C1D5F] to-[#84117C] text-white p-6 md:p-8 rounded-3xl shadow-xl dark:shadow-purple-900/20 border border-white/10 dark:border-white/5 relative overflow-hidden flex flex-col justify-center min-h-[160px]">
        ...
        <div className="relative z-10 space-y-1.5 max-w-3xl">
          ...
          <p className="text-primary text-xs md:text-sm leading-relaxed font-medium mt-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm p-4 md:p-5 rounded-2xl flex items-center gap-4 hover:-translate-y-1 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          <div className="p-3 bg-[#01AC9F]/10 dark:bg-accent-2 text-[#01AC9F] rounded-2xl shrink-0">
        ...
        {/* KPI 2 */}
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm p-4 md:p-5 rounded-2xl flex items-center gap-4 hover:-translate-y-1 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          <div className="p-3 bg-accent-2/10 text-accent-2 rounded-2xl shrink-0">
        ...
        {/* KPI 3 */}
        <div className="bg-white dark:bg-neutral-900 border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm p-4 md:p-5 rounded-2xl flex items-center gap-4 hover:-translate-y-1 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
          <div className="p-3 bg-primary/10 text-brand-velvet rounded-2xl shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs bg-destructive/10 text-rose-700 font-bold px-2.5 py-1 rounded-full animate-pulse">{activeAssessments.length} Pending Assessment</span>
          </div>
                  ...
                  <div key={as.id} className="bg-white dark:bg-neutral-900 p-6 rounded-3xl border border-brand-border dark:border-neutral-700 dark:border-neutral-700/60 shadow-sm space-y-4 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start gap-1">
                        <span className="px-2 py-0.5 bg-accent-2/10 text-[#01AC9F] font-bold rounded text-[10px] uppercase font-mono">
                          {as.type.replace('_', ' ')}
                        </span>
                        <span className={`text-[10px] font-bold ${as.difficulty === 'Easy' ? 'text-accent-2' : as.difficulty === 'Medium' ? 'text-destructive' : 'text-rose-500'}`}>
                          {as.difficulty}
                        </span>
                      </div>
            ...
            {recentScores.length === 0 ?
              ...
              recentScores.map((score) =>
                <div
                  ...
                  className="p-3 bg-neutral-50 dark:bg-neutral-950 rounded-2xl border border-brand-border/20 hover:border-brand-velvet dark:hover:border-primary transition-all cursor-pointer flex items-center justify-between gap-3">
                  ...
                  <span className={`px-2.5 py-1.5 rounded-2xl font-mono font-black text-xs shrink-0 text-center ${score.isEvaluated ? 'bg-accent-2/10 text-[#01AC9F] dark:bg-accent-2 dark:text-accent-2' : 'bg-destructive/10 text-destructive dark:bg-destructive'}`}>
