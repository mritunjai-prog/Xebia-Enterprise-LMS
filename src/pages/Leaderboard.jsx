...
export const Leaderboard = () => {
  ...
  const podiumLayout = [];
  if (podiumEntries[1])
    podiumLayout.push({ entry: podiumEntries[1], rank: 2, config: medalStyles.second });
  if (podiumEntries[0])
    podiumLayout.push({ entry: podiumEntries[0], rank: 1, config: medalStyles.first });
  if (podiumEntries[2])
    podiumLayout.push({ entry: podiumEntries[2], rank: 3, config: medalStyles.third });
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-neutral-500 dark:text-neutral-400">
          Honoring the top academic performers, completed assessments, and average grades
        </p>
      </div>
      ...
      {podiumEntries.length > 0 && (
        <div className="flex flex-row justify-center items-end gap-2 md:gap-16 pt-24 md:pt-32 pb-12 px-4 bg-transparent dark:bg-[#0a0a0a]/50 rounded-3xl shadow-none dark:shadow-inner mt-4">
          {podiumLayout.map(({ entry, rank, config }) => (
            <div
              key={entry.studentId}
              className={`flex flex-col items-center w-28 md:w-40 ${config.order}`}
            >
              <motion.div
              ...
              >
                ...
                <div className="relative w-32 h-32 flex items-center justify-center">
                  ...
                  <div
                    className={`absolute inset-0 rounded-full border-[4px] border-dashed ${config.wreath} scale-[1.12] opacity-60 rotate-45`}
                  ></div>
                  <div
                    className={`absolute inset-0 rounded-full border-[1px] ${config.wreath} scale-[1.20] opacity-30`}
                  ></div>
                  <div
                    className={`absolute inset-0 rounded-full border-[2px] border-dotted ${config.wreath} scale-[1.28] opacity-20`}
                  ></div>

                  {/* Stars Arch */}
                  <div
                    className={`absolute -top-7 flex gap-1 justify-center w-full ${config.text} drop-shadow-md`}
                  >
                  ...
                  {/* The Coin */}
                  <div
                    className={`relative w-28 h-28 rounded-full ${config.coinBg} shadow-[0_0_20px_rgba(0,0,0,0.5)] border-[6px] ${config.coinBorder} flex items-center justify-center z-10`}
                  >
                    {/* Inner Detail */}
                    <div
                      className={`absolute inset-2 rounded-full border-[2px] border-dashed ${config.innerRing} opacity-40`}
                    ></div>
                    <div
                      className={`absolute inset-1 rounded-full border-[1px] ${config.innerRing} opacity-20`}
                    ></div>

                    {/* Number */}
                    <span
                      className={`text-6xl font-black ${config.text} drop-shadow-sm`}
                  ...
                  <div className="absolute -bottom-5 z-20 flex flex-col items-center w-[120%]">
                    ...
                    {/* Ribbon main body */}
                    <div
                      className={`relative px-4 py-1.5 w-full ${config.ribbonBg} shadow-[0_4px_10px_rgba(0,0,0,0.3)] z-20 flex justify-center items-center rounded-sm`}
                    >
                      <div className="absolute inset-0 border-y border-white/20 pointer-events-none"></div>
                      <span
                        className={`${config.ribbonText} font-black text-[10px] md:text-xs uppercase tracking-wider truncate drop-shadow-sm`}
                      >
                        {entry.studentName}
                      </span>
                    </div>
                ...
                <div className="mt-12 flex flex-col items-center">
                  <img
                    src={entry.avatar}
                    className={`w-10 h-10 rounded-full border-2 ${config.wreath} shadow-lg object-cover bg-white`}
                    alt={entry.studentName}
                  />
                  <p className="font-mono font-black text-neutral-800 dark:text-white mt-2 text-sm">
                    {entry.score} pts
                  </p>
                </div>
              </motion.div>
