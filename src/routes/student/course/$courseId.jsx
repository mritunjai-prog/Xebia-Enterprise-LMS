...
import {
  ...
  FileText, PlayCircle, Image as ImageIcon, Code, Link as LinkIcon, File, Play, X, Download, Layers, Activity
} from "lucide-react";
...
function ContentRenderer({ block }) {
  ...
  switch (uiType) {
    ...
    case "LINK":
      return (
        <div className="p-6 rounded-2xl bg-[#6C1D5F]/5 dark:bg-[#84117C]/10 border border-[#6C1D5F]/20 dark:border-[#84117C]/30 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-lg font-bold text-[#000000] dark:text-[#FFFFFF] mb-2">External Resource</h3>
          <a href={dataToRender} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#6C1D5F] hover:bg-[#4A1E47] text-[#FFFFFF] text-sm font-bold rounded-xl shadow transition-all">
    ...
    case "Heading": {
      ...
      const sizeClass = parsedData.headingLevel === 1 ? 'text-4xl' : parsedData.headingLevel === 2 ? 'text-3xl' : 'text-2xl';
      return (
        <HeadingTag className={clsx(`font-extrabold text-[#000000] dark:text-[#FFFFFF] my-4`, sizeClass)}>
          {dataToRender}
        </HeadingTag>
      );
    }
    ...
    case "Table":
          ...
          return (
            <div className="overflow-x-auto w-full my-6 rounded-xl border border-[#2e2e3e] shadow-sm bg-white dark:bg-[#15151f]">
              <table className="w-full text-left text-sm text-gray-700 dark:text-gray-300">
                ...
                <tbody>
                  {tableData.map((row, i) => (
                    <tr key={i} className="border-b border-gray-100 dark:border-[#2e2e3e] last:border-0 hover:bg-gray-50 dark:hover:bg-[#1a1a24] transition-colors">
                      {headers.map((h, j) => (
                        <td key={j} className="px-6 py-4 whitespace-pre-wrap">{row[h]}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
      ...
      return (
        <div className="prose prose-slate dark:prose-invert max-w-none">
          <div className="whitespace-pre-wrap font-mono text-sm text-gray-500 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">{dataToRender || 'No table data. Add JSON array in Admin.'}</div>
        </div>
      );
    case "Text":
    case "NOTE":
...
function CourseViewer() {
  ...
  const isEnrolled = enrollmentStatus?.isEnrolled || (enrolledCoursesData || []).some(c => String(c.id) === String(courseId));
        ...
        <div className="bg-white dark:bg-[#15151f] border border-[#DEDEDE] dark:border-[#5A5A5A] rounded-3xl shadow-xl overflow-hidden">
          ...
          <div className="w-full aspect-video md:aspect-[21/9] bg-[#F7F8FC] dark:bg-[#15151f] relative border-b border-[#DEDEDE] dark:border-[#5A5A5A]">
            {(course.image || course.thumbnailImageUrl || course.thumbnail || course.icon) ? (
              <img src={course.image || course.thumbnailImageUrl || course.thumbnail || course.icon} alt={course.title} className="w-full h-full object-cover" />
            ) : (
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-4 py-6 border-y border-[#DEDEDE] dark:border-[#5A5A5A]">
                <div className="flex items-center gap-2 text-sm font-bold text-[#000000] dark:text-[#FFFFFF] bg-[#F7F8FC] dark:bg-white/5 px-4 py-2 rounded-xl">
                  ...
                  {(`${course.durationHours ? course.durationHours + 'h ' : ''}${course.durationMinutes ? course.durationMinutes + 'm' : ''}`.trim()) || course.duration || "Self Paced"}
                </div>
                <div className="flex items-center gap-2 text-sm font-bold text-[#000000] dark:text-[#FFFFFF] bg-[#F7F8FC] dark:bg-white/5 px-4 py-2 rounded-xl">
                ...
                <div className={clsx(
                  ...
                  (course.difficultyLevel || course.level || 'Beginner') === 'Beginner' ? 'bg-[#01AC9F] shadow-[0_0_15px_rgba(1,172,159,0.3)]' : 
                  (course.difficultyLevel || course.level || 'Beginner') === 'Intermediate' ? 'bg-[#84117C] shadow-[0_0_15px_rgba(132,17,124,0.3)]' : 
              ...
              <p className="text-lg text-[#5A5A5A] dark:text-[#DADCEA] leading-relaxed text-left whitespace-pre-wrap">
                {course.description || course.shortDescription || "Join this course to unlock premium learning materials and enhance your skills."}
              </p>
    ...
    <div className="max-w-[1600px] mx-auto pb-12 h-full flex flex-col animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-extrabold text-[#000000] dark:text-[#FFFFFF] leading-tight line-clamp-1">{course.title}</h1>
          </div>
        </div>
        <div className="hidden sm:flex items-center gap-4 bg-white dark:bg-white/5 px-5 py-2.5 rounded-2xl border border-[#DEDEDE] dark:border-white/10 shadow-sm">
          <div className="text-right">
            <p className="text-[10px] font-bold text-[#5A5A5A] dark:text-[#DADCEA] uppercase tracking-wider mb-0.5">Your Progress</p>
            <p className="text-sm font-extrabold text-[#000000] dark:text-[#FFFFFF] leading-none">{progressPercent}%</p>
          </div>
          <div className="w-32">
            <Progress value={progressPercent} className="h-2 [&>div]:bg-[#6C1D5F] dark:[&>div]:bg-[#84117C] bg-[#F7F8FC] dark:bg-[#15151f]" />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
              ...
              <>
                <div className="p-6 sm:p-10 flex-1 space-y-8 bg-white dark:bg-[#15151f]">
                  ...
                  >
                    ...
                    <h2 className="text-2xl sm:text-4xl font-black text-[#000000] dark:text-[#FFFFFF]">{activeSubmodule.title}</h2>
                  </motion.div>
                  ...
                  {(activeSubmodule.contentBlocks || []).length > 0 ? (
                    ...
                    >
                      {[...activeSubmodule.contentBlocks].sort((a, b) => (a.sequenceOrder ?? a.position ?? 0) - (b.sequenceOrder ?? b.position ?? 0)).map((block, idx) => (
                        <motion.div 
                          key={block.id}
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="py-20 text-center">
                      ...
                      <h3 className="text-lg font-bold text-[#000000] dark:text-[#FFFFFF]">Empty Lesson</h3>
                      <p className="text-[#5A5A5A] dark:text-[#DADCEA] mt-1">No content has been added to this lesson yet.</p>
                    </div>
                  )}
                </div>
        ...
        <div className="w-full lg:w-[380px] xl:w-[420px] shrink-0">
          <div className="bg-white dark:bg-[#15151f] border border-[#DEDEDE] dark:border-white/10 rounded-3xl shadow-lg lg:sticky lg:top-6 overflow-hidden flex flex-col" style={{ maxHeight: "calc(100vh - 48px)" }}>
            <div className="p-6 border-b border-[#DEDEDE] dark:border-white/10 bg-[#F7F8FC] dark:bg-white/5 shrink-0">
              ...
              <p className="text-xs font-medium text-[#5A5A5A] dark:text-[#DADCEA] mt-1">{modules.length} Modules • {totalLessons} Lessons</p>
            </div>
              ...
              {modules.length > 0 ? modules.map((mod, mIdx) => {
                ...
                return (
                  <div key={mod.id} className={clsx("rounded-2xl overflow-hidden transition-all duration-300", 
                    isActiveModule ? "border-[#6C1D5F]/30 dark:border-[#84117C]/30 shadow-md ring-1 ring-[#6C1D5F]/20 dark:ring-[#84117C]/20" : "border border-[#DEDEDE] dark:border-white/10"
                  )}>
                    <button
                      onClick={() => toggleModule(mod.id)}
                      className={clsx("w-full flex items-center gap-3 p-4 transition-colors text-left",
                        isActiveModule ? "bg-[#6C1D5F]/5 dark:bg-[#84117C]/10" : "bg-white dark:bg-[#15151f] hover:bg-[#F7F8FC] dark:hover:bg-white/5"
                      )}
                    >
                      <div className="flex-1 min-w-0 pr-2">
                        <p className={clsx("font-extrabold text-sm leading-tight line-clamp-2", isActiveModule ? "text-[#6C1D5F] dark:text-[#84117C]" : "text-[#000000] dark:text-[#FFFFFF]")}>
                      ...
                      <ChevronDown className={clsx("w-4 h-4 shrink-0 transition-transform duration-300", isActiveModule ? "text-[#6C1D5F] dark:text-[#84117C]" : "text-[#DADCEA] dark:text-[#5A5A5A]", isExpanded && "rotate-180")} />
                    </button>
                                ...
                                >
                                  <div className="shrink-0 flex items-center justify-center">
                                    {completedSubmoduleIds.includes(sub.id) ? (
                                      <CheckCircle2 className={clsx("w-5 h-5", isActive ? "text-[#FFFFFF]" : "text-[#01AC9F]")} />
                                    ) : isActive ? (
                                  ...
                                  <span className={clsx("text-sm font-bold truncate flex-1", isActive ? "text-[#FFFFFF]" : "text-[#5A5A5A] dark:text-[#DADCEA] group-hover:text-[#000000] dark:group-hover:text-[#FFFFFF]")}>
                                    {sub.title}
                                  </span>
                                </button>
                              );
