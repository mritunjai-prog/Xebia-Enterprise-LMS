...
import { upcomingAssessments } from "@/lib/dummy-data";
...
function AssessmentsPage() {
  ...
  const AssessmentCard = ({ assessment, idx }) => {
    ...
    return (
      ...
      >
        {/* Top Banner Area */}
        <div className="relative h-32 bg-gray-100 dark:bg-gray-800 overflow-hidden shrink-0 p-4 flex items-start justify-between border-b border-gray-200 dark:border-[#2e2e3e]">
        ...
        <div className="p-5 flex-1 flex flex-col">
          <p className="text-xs font-bold text-[#6C1D5F] dark:text-[#b44e9f] uppercase tracking-wider mb-1 line-clamp-1">
            {assessment.course}
          </p>
          <h2 className="text-lg font-extrabold text-gray-900 dark:text-white leading-tight mb-4 line-clamp-2 group-hover:text-[#6C1D5F] dark:group-hover:text-[#b44e9f] transition-colors">
            {assessment.name}
          </h2>

          <div className="mt-auto space-y-4">
            <div className="flex items-center gap-2 text-sm font-medium text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 p-2.5 rounded-lg border border-gray-100 dark:border-gray-800">
              <Calendar className="w-4 h-4 shrink-0 text-gray-400" />
              <span>Scheduled for <strong className="text-gray-700 dark:text-gray-300">{assessment.date}</strong></span>
            </div>

            {/* CTA Button */}
            {assessment.status === "Completed" ? (
              <Link to="/student/results" className="block w-full">
                <button className="w-full py-2.5 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 bg-gray-100 dark:bg-[#252535] text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-[#2e2e3e]">
                  <Award className="w-4 h-4" /> View Results
                </button>
              </Link>
            ) : assessment.status === "Upcoming" ? (
              <button disabled className="w-full py-2.5 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 cursor-not-allowed">
    ...
    if (items.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-16 px-6 text-center bg-white dark:bg-[#15151f] rounded-2xl border border-gray-200 dark:border-[#2e2e3e] shadow-sm">
          ...
          <p className="text-lg font-bold text-gray-900 dark:text-white mb-1">No assessments found</p>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="relative w-full sm:w-64 shrink-0">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-xl text-sm font-medium text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:ring-2 focus:ring-[#6C1D5F]/20 focus:border-[#6C1D5F] transition-all shadow-sm"
            />
          </div>
        </div>
      </div>
        ...
        <TabsList className="mb-8 h-auto p-1.5 bg-gray-100 dark:bg-[#15151f] border border-transparent dark:border-[#2e2e3e] w-full sm:w-auto justify-start overflow-x-auto rounded-xl">
          {[
            ...
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className="px-5 py-2 rounded-lg font-bold text-sm data-[state=active]:bg-white dark:data-[state=active]:bg-[#252535] data-[state=active]:text-[#6C1D5F] dark:data-[state=active]:text-white data-[state=active]:shadow-sm text-gray-500 dark:text-gray-400 whitespace-nowrap transition-all"
            >
              {tab.label}
            </TabsTrigger>
          ))}
