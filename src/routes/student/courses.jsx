  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Access your assigned courses and continue learning.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {enrolledCourses.map((course) => (
          <Card key={course.id} className="glass group overflow-hidden flex flex-col h-full hover:shadow-elegant transition-all duration-300">
            ...
            <CardContent className="flex-1 p-5 py-2">
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Progress ({course.modulesCompleted}/{course.totalModules} Modules)</span>
                ...
                {course.progress > 0 && course.progress < 100 && (
                  <div className="flex items-start gap-2 bg-muted/40 p-3 rounded-lg mt-4 border border-border/50">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mb-0.5">Resume from</p>
            ...
            <CardFooter className="p-5 pt-4">
              ...
              >
                <Link to={`/student/course/${course.id}`}>
                  {course.progress === 0 ? "Start Course" : course.progress === 100 ? "View Certificate" : "Continue Learning"}
                </Link>
              </Button>
            </CardFooter>
          </Card>
