...
function ExamTakingView() {
  ...
  if (isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] animate-in fade-in zoom-in duration-500">
        ...
        <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2">Assessment Submitted</h1>
        <p className="text-gray-500 dark:text-gray-400">Your answers have been saved securely.</p>
        <div className="mt-8 flex items-center gap-2 text-sm font-bold text-[#6C1D5F] dark:text-[#b44e9f] animate-pulse">
  ...
  return (
    <div className="max-w-5xl mx-auto py-4 space-y-6 animate-in fade-in duration-500">
      ...
      {/* ── Top Header Panel ── */}
      <div className="bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-2xl p-5 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">React Final Exam</h1>
          <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mt-1">ID: {assessmentId}</p>
        </div>
        
        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Time Remaining</span>
            <div className={clsx("flex items-center gap-2 text-3xl font-black font-mono", timeLeft < 300 ? "text-red-500 animate-pulse" : "text-gray-800 dark:text-gray-200")}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        {/* ── Main Question Area ── */}
        <div className="lg:col-span-3 bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-2xl shadow-sm flex flex-col min-h-[500px]">
          
          {/* Progress Bar */}
          <div className="h-1.5 w-full bg-gray-100 dark:bg-gray-800 rounded-t-2xl overflow-hidden">
            <div className="h-full bg-[#6C1D5F] transition-all duration-500 ease-out" style={{ width: `${progressPercent}%` }} />
          </div>

          <div className="p-8 flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-8">
              <span className="text-sm font-extrabold text-[#6C1D5F] dark:text-[#b44e9f] uppercase tracking-widest bg-[#6C1D5F]/10 dark:bg-[#b44e9f]/10 px-3 py-1.5 rounded-lg">
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white leading-relaxed mb-8">
              {currentQ.question}
            </h2>
                    ...
                    >
                      <button
                        ...
                        className={clsx(
                          ...
                          isSelected 
                            ? "border-[#6C1D5F] bg-[#6C1D5F]/5 dark:bg-[#6C1D5F]/10 shadow-[0_0_0_4px_rgba(108,29,95,0.1)]" 
                            : "border-gray-200 dark:border-[#2e2e3e] hover:border-[#6C1D5F]/30 bg-transparent hover:bg-gray-50 dark:hover:bg-[#1a1a24]"
                        )}
                      >
                        <div className={clsx(
                          "w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 transition-colors",
                          isSelected ? "border-[#6C1D5F]" : "border-gray-300 dark:border-gray-600 group-hover:border-[#6C1D5F]/50"
                        )}>
                          {isSelected && <div className="w-3 h-3 bg-[#6C1D5F] rounded-full" />}
                        </div>
                        <span className={clsx(
                          "text-lg font-medium",
                          isSelected ? "text-[#6C1D5F] dark:text-[#b44e9f]" : "text-gray-700 dark:text-gray-300"
                        )}>
                          {option}
                        </span>
                      </button>
          ...
          {/* Navigation Footer */}
          <div className="p-5 border-t border-gray-100 dark:border-[#2e2e3e] flex items-center justify-between bg-gray-50 dark:bg-[#1a1a24] rounded-b-2xl">
            <button
              onClick={() => setCurrentIdx(prev => prev - 1)}
              disabled={isFirstQ}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
        ...
        <div className="bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-2xl p-5 shadow-sm sticky top-6">
          <h3 className="font-extrabold text-gray-900 dark:text-white mb-4">Questions</h3>
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    isAttempted 
                      ? "bg-[#6C1D5F] text-white border-transparent" 
                      : "bg-transparent border-2 border-gray-200 dark:border-[#2e2e3e] text-gray-500 dark:text-gray-400"
                  )}
                >
                  {idx + 1}
                </button>
          ...
          <div className="mt-6 pt-5 border-t border-gray-100 dark:border-[#2e2e3e] space-y-3">
            <div className="flex items-center justify-between text-sm">
              <span className="flex items-center gap-2 text-gray-500 dark:text-gray-400 font-medium">
                <div className="w-3 h-3 bg-[#6C1D5F] rounded-full" /> Attempted
              </span>
              <span className="font-bold text-gray-900 dark:text-white">{attemptedCount}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="flex items-center gap-2 text-gray-500 dark:text-gray-400 font-medium">
                <div className="w-3 h-3 border-2 border-gray-300 dark:border-[#2e2e3e] rounded-full" /> Unattempted
              </span>
              <span className="font-bold text-gray-900 dark:text-white">{dummyQuestions.length - attemptedCount}</span>
            </div>
          </div>
        </div>
