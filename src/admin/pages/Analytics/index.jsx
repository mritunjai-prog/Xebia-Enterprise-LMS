...
import { 
  Eye, MousePointerClick, Target, TrendingUp, Calendar, Filter
} from 'lucide-react';
import { CourseService, CategoryService } from '../../../services/api';

const COLORS = ['#6C1D5F', '#01AC9F', '#FF6200', '#F4A261', '#E76F51', '#34d399', '#a78bfa', '#f97316'];
...
export default function Analytics() {
  ...
  const categoryDistribution = Object.keys(catCount).map(name => ({
    name, value: catCount[name]
  ...
  courses.forEach(c => {
    if (c.createdAt) {
      ...
      if (!monthlyActivityMap[month]) monthlyActivityMap[month] = { name: month, views: 0, clicks: 0 };
  ...
  const monthlyActivity = monthNames.map(m => monthlyActivityMap[m] || { name: m, views: 0, clicks: 0 });
  
  // Sort years
  const yearlyActivity = Object.values(yearlyActivityMap).sort((a, b) => a.name.localeCompare(b.name));
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Platform Analytics</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#151515] rounded-xl border border-gray-200 dark:border-[#333] shadow-sm p-5">
          ...
          <h3 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{totalViews.toLocaleString()}</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold mt-1 uppercase tracking-wider">Total Views</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#151515] rounded-xl border border-gray-200 dark:border-[#333] shadow-sm p-5">
          ...
          <h3 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{totalClicks.toLocaleString()}</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold mt-1 uppercase tracking-wider">Total Clicks</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#151515] rounded-xl border border-gray-200 dark:border-[#333] shadow-sm p-5">
          ...
          <h3 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{avgCtr}%</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold mt-1 uppercase tracking-wider">Avg. CTR</p>
        </div>
        ...
        <div className="bg-white dark:bg-[#151515] rounded-xl border border-gray-200 dark:border-[#333] shadow-sm p-5">
          ...
          <h3 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{avgSeoScore}/100</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold mt-1 uppercase tracking-wider">Avg. SEO Score</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#151515] rounded-xl border border-gray-200 dark:border-[#333] shadow-sm p-6">
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Views vs Clicks ({timeRange})</p>
            </div>
          ...
          <div className="h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={activityData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#888' }} dx={-10} tickFormatter={(val) => val > 1000 ? `${(val / 1000).toFixed(1)}k` : val} />
                <RechartsTooltip content={<CustomTooltip />} />
                <Legend iconType="circle" wrapperStyle={{ paddingTop: '20px', fontSize: '12px' }} />
                <Line type="monotone" name="Total Views" dataKey="views" stroke="#6C1D5F" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                <Line type="monotone" name="Total Clicks" dataKey="clicks" stroke="#01AC9F" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            {categoryDistribution.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <RechartsTooltip 
                    contentStyle={{ borderRadius: '8px', border: '1px solid #333', backgroundColor: 'var(--tw-colors-gray-900)', color: '#fff' }}
                    itemStyle={{ color: '#fff', fontSize: '14px', fontWeight: 'bold' }}
                  />
                </PieChart>
              </ResponsiveContainer>
            ) : (
               <div className="flex items-center justify-center h-full text-sm text-gray-500">No categories found.</div>
            )}
          </div>
          <div className="grid grid-cols-2 gap-x-2 gap-y-3 mt-4">
            {categoryDistribution.map((entry, index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-600 dark:text-gray-400 truncate" title={entry.name}>{entry.name} ({entry.value})</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#151515] rounded-xl border border-gray-200 dark:border-[#333] shadow-sm p-6">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Based on highest total views and clicks</p>
        </div>
        <div className="h-[350px] w-full">
          {topCourses.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topCourses} margin={{ top: 20, right: 30, left: 20, bottom: 5 }} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#333" strokeOpacity={0.2} />
                <XAxis type="number" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#888' }} />
                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#888', fontWeight: 600 }} width={120} />
                <RechartsTooltip cursor={{fill: 'transparent'}} content={<CustomTooltip />} />
                <Legend iconType="circle" wrapperStyle={{ paddingTop: '10px', fontSize: '12px' }} />
                <Bar dataKey="views" name="Total Views" fill="#6C1D5F" radius={[0, 4, 4, 0]} barSize={20} />
                <Bar dataKey="clicks" name="Total Clicks" fill="#F4A261" radius={[0, 4, 4, 0]} barSize={20} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
             <div className="flex items-center justify-center h-full text-sm text-gray-500">No courses available.</div>
          )}
        </div>
      </div>
