...
import {
  Search, Plus, LayoutGrid, List as ListIcon, Layers,
  Edit3, Trash2, Star, ChevronLeft, ChevronRight,
  Clock, BookOpen, Globe, Filter, MoreVertical,
  GraduationCap, FileText, Bookmark, Users,
...
export default function Courses() {
  ...
  const filtered = courses.filter(c => {
    ...
    if (search && !(c.title || '').toLowerCase().includes(search.toLowerCase()) &&
        !(c.courseCode || c.slug || '').toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
    ...
    return (
      <CreateCourse
        ...
        onBack={() => {
          setShowCreate(false);
          setEditData(null);
          CourseService.getCourses().then(d => setCourses(d || [])).catch(() => {});
        }}
        onSaved={(course) => {
          if (editData) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col gap-6 mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Courses</h1>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mt-1">Manage and organize your learning library.</p>
            </div>
          </div>
          <button
      ...
      <div className="bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-2xl shadow-sm p-2 flex flex-col xl:flex-row items-center justify-between gap-2">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 w-full xl:w-auto overflow-x-auto hide-scrollbar">
          ...
          <div className="relative shrink-0">
            <select
              ...
              onChange={e => { setFilterFavorites(e.target.value); setCurrentPage(1); }}
            ...
            >
              {['All Courses', 'Favorites'].map(o => <option key={o}>{o}</option>)}
            </select>
        ...
        <div className="flex items-center gap-2 w-full xl:w-auto overflow-x-auto pb-2 xl:pb-0 hide-scrollbar shrink-0 justify-start xl:justify-end">
          ...
          {[
            ...
            { value: filterCat, set: setFilterCat, opts: ['All Categories', ...categories.map(c => c.name)] },
            { value: filterStatus, set: setFilterStatus, opts: ['All Status', 'Published', 'Draft', 'Active', 'Inactive'] },
          ].map(({ value, set, opts }) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <AnimatePresence>
            {paginated.map((course, idx) => {
              ...
              const duration = (`${course.durationHours ? course.durationHours + 'h ' : ''}${course.durationMinutes ? course.durationMinutes + 'm' : ''}`.trim()) || course.duration || 'N/A';
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5 flex-1 flex flex-col relative">
                    ...
                    <div className="mb-2">
                        <span
                          className="text-xl font-extrabold text-gray-900 dark:text-white group-hover:text-[#6C1D5F] dark:group-hover:text-[#84117C] transition-colors leading-tight block mb-2 line-clamp-2"
                        >
                          {course.title}
                        </span>
                      </div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2 leading-relaxed mb-6 font-medium">
                      {course.description || course.shortDescription || 'No description available for this course. Click to learn more.'}
                    </p>
                    ...
                    <div className="mt-auto flex items-center justify-between text-[10px] sm:text-[11px] font-bold text-gray-500 dark:text-gray-400">
                      ...
                      <div className="relative group/menu">
                        ...
                        <div className="absolute bottom-full right-0 mb-2 w-36 bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-xl shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all flex flex-col overflow-hidden z-10">
                          <button onClick={(e) => { e.stopPropagation(); setEditData(course); setShowCreate(true); }} className="px-4 py-2 text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-[#252535]">
                            <Edit3 className="w-3.5 h-3.5" /> Edit
                          </button>
                          <button onClick={(e) => { e.stopPropagation(); handleToggleStatus(course, 'active'); }} className="px-4 py-2 text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-[#252535]">
                            {active ? <XCircle className="w-3.5 h-3.5" /> : <CheckCircle className="w-3.5 h-3.5" />}
                            {active ? 'Deactivate' : 'Activate'}
                          </button>
                          <button onClick={(e) => { e.stopPropagation(); handleToggleStatus(course, 'publish'); }} className="px-4 py-2 text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-[#252535]">
            ...
            <thead>
              <tr className="bg-gray-50/80 dark:bg-[#1e1e2d] border-b border-gray-200 dark:border-[#2e2e3e]">
                {['Course', 'Category', 'Level', 'Duration', 'Status', 'Actions'].map(h => (
                  <th key={h} className={clsx('px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider', h === 'Actions' && 'text-right')}>
                    {h}
                  </th>
                ))}
              </tr>
                ...
                {paginated.map((course, idx) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-xl overflow-hidden shrink-0 bg-gray-100 dark:bg-gray-800 shadow-sm flex items-center justify-center text-gray-400 relative">
                            <div className="absolute inset-0 flex items-center justify-center z-0">
                              <span className="text-lg font-bold opacity-30 uppercase">{course.title ? course.title.substring(0, 2) : 'CO'}</span>
                            </div>
                            {thumb && (
                              <img src={thumb} alt="" className="w-full h-full object-contain p-1 relative z-10" onError={e => { e.target.style.display = 'none'; }} />
                            )}
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        {cat !== '—' ? (
                          <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-md text-white shadow-sm" style={{ backgroundColor: catColor(cat, catIdx >= 0 ? catIdx : idx) }}>
                            {cat}
                          </span>
                      ...
                      {/* Level */}
                      <td className="px-6 py-4">
                        <span className={clsx('inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-md ring-1 ring-inset', levelStyle.bg)}>
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-gray-600 dark:text-gray-300">
                        <div className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-gray-400 dark:text-gray-500" />{duration}</div>
                      </td>
      ...
      <div className="flex items-center justify-between pt-2">
        <span className="text-sm text-gray-500 dark:text-gray-400 font-medium">
          Showing {filtered.length === 0 ? 0 : (currentPage - 1) * ITEMS_PER_PAGE + 1} to {Math.min(currentPage * ITEMS_PER_PAGE, filtered.length)} of {filtered.length} courses
        </span>
        {totalPages > 1 && (
          <div className="flex items-center gap-1.5 bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-xl p-1 shadow-sm">
            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-[#252535] hover:text-gray-900 dark:hover:text-white disabled:opacity-40 disabled:hover:bg-transparent transition-colors">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <div className="flex items-center px-2">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setCurrentPage(p)}
                  className={clsx('w-8 h-8 rounded-lg text-sm font-bold transition-colors mx-0.5', p === currentPage ? 'bg-[#6C1D5F] text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#252535]')}
                >
                  {p}
                </button>
              ))}
            </div>
            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-[#252535] hover:text-gray-900 dark:hover:text-white disabled:opacity-40 disabled:hover:bg-transparent transition-colors">
