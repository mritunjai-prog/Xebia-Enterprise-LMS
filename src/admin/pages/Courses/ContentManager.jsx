...
import { ChevronLeft, Plus, Settings, Video, FileText, Code, Link as LinkIcon, Trash2, Edit3, GripVertical, CheckCircle2, X, Image as ImageIcon, LayoutTemplate, Table } from 'lucide-react';
...
export default function ContentManager({ submoduleId, courseId, moduleId }) {
  ...
  const inputCls = "w-full px-3.5 py-2.5 bg-background dark:bg-[#1a1a24] border border-border dark:border-[#3a3a4a] rounded-xl text-sm focus:outline-none transition-colors text-foreground dark:text-white";
    ...
    <div className="flex flex-col bg-[#F7F8FC] dark:bg-[#0a0a0f] transition-colors rounded-2xl border border-gray-200 dark:border-[#2e2e3e] overflow-hidden" style={{ height: 'calc(100vh - 40px)' }}>
      <div className="h-14 border-b border-border dark:border-[#2e2e3e] bg-white dark:bg-[#15151f] flex items-center justify-between px-6 shrink-0 shadow-sm">
        <div className="flex items-center gap-3">
          <button onClick={() => router.history.back()} className="h-8 w-8 rounded-lg border border-border dark:border-[#2e2e3e] flex items-center justify-center hover:bg-gray-50 dark:hover:bg-[#1a1a24] cursor-pointer"><ChevronLeft className="w-4 h-4"/></button>
          <div className="h-5 w-px bg-border" />
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: BRAND }}>Content Management</span>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 flex flex-col border-r border-border dark:border-[#2e2e3e] bg-[#fafafa] dark:bg-[#0d0d14]">
          <div className="px-6 py-4 flex items-center justify-between bg-white dark:bg-[#15151f] border-b border-border dark:border-[#2e2e3e] shrink-0">
            <h2 className="text-lg font-bold text-foreground dark:text-white">Existing Content Blocks</h2>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {isLoading ? (
              ...
              return (
                <div key={block.id} className="bg-white dark:bg-[#15151f] rounded-2xl border border-gray-200 dark:border-[#2e2e3e] shadow-sm flex items-center p-4 gap-4">
                  <div className="cursor-grab text-gray-400 hover:text-gray-600"><GripVertical className="w-5 h-5"/></div>
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style={{ background: BRAND_LIGHT, color: BRAND }}><Icon className="w-5 h-5"/></div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full" style={{ background: BRAND_LIGHT, color: BRAND }}>{typeDef.label}</span>
                      <span className="text-xs text-gray-400">Order: {block.position}</span>
                    </div>
                    <h3 className="text-sm font-bold text-foreground dark:text-white truncate">{block.title}</h3>
                  </div>
                  <div className="flex items-center gap-3">
          ...
          {showEditor ? (
            <form onSubmit={handleSave} className="flex-1 flex flex-col h-full overflow-hidden">
              <div className="px-6 py-4 flex items-center justify-between border-b border-border dark:border-[#2e2e3e] shrink-0 bg-white dark:bg-[#15151f]">
                <h2 className="text-lg font-bold text-foreground dark:text-white">{editId ? 'Edit Block' : 'New Block'}</h2>
                <button type="button" onClick={() => setShowEditor(false)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5"/></button>
              </div>
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                
                <div>
                  <label className="text-xs font-bold text-muted-foreground uppercase block mb-2">Content Type</label>
                  <div className="flex flex-wrap gap-2">
                    {BLOCK_TYPES.map(type => {
                      const isSelected = activeType === type.id;
                      const Icon = type.icon;
                      return (
                        <button key={type.id} type="button" onClick={() => setActiveType(type.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${isSelected ? 'text-white' : 'text-gray-600 bg-white border-gray-200 hover:bg-gray-50'}`} style={{ background: isSelected ? BRAND : undefined, borderColor: isSelected ? BRAND : undefined }}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="text" className={inputCls} value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="Block title" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-muted-foreground uppercase block mb-1.5">Content Order</label>
                    <input type="number" min="1" className={inputCls} value={formData.contentOrder} onChange={e => setFormData({...formData, contentOrder: parseInt(e.target.value)})} />
                  </div>
                </div>
                ...
                {(activeType === 'Text' || activeType === 'Callout') && (
                  <div className="space-y-4">
                    {activeType === 'Text' && (
                      <div>
                        <label className="text-xs font-bold text-muted-foreground uppercase block mb-1.5">Import from .TXT (Optional)</label>
                        <FileUpload accept=".txt" onUpload={(file) => {
                           const reader = new FileReader();
                           reader.onload = (e) => setFormData({...formData, text: e.target.result});
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" className={inputCls} value={formData.alt} onChange={e => setFormData({...formData, alt: e.target.value})} />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-muted-foreground uppercase block mb-1.5">Caption</label>
                        <input type="text" className={inputCls} value={formData.caption} onChange={e => setFormData({...formData, caption: e.target.value})} />
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    {renderToggle(formData.isActive, e => setFormData({...formData, isActive: e.target.checked}))}
                    <span className="text-sm font-bold" style={{ color: formData.isActive ? BRAND : '#9ca3af' }}>Active</span>
                  </div>
                </div>

              </div>
              <div className="px-6 py-4 bg-gray-50 dark:bg-[#1a1a24] border-t border-border flex justify-end gap-3 shrink-0">
                <button type="button" onClick={() => setShowEditor(false)} className="px-4 py-2 rounded-xl text-sm font-bold border border-gray-300 hover:bg-gray-100">Cancel</button>
                <button type="submit" className="px-4 py-2 rounded-xl text-sm font-bold text-white shadow-sm" style={{ background: BRAND }}>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8 text-center">
              ...
              <h3 className="font-bold text-lg text-gray-600 dark:text-gray-300">Select a Block to Edit</h3>
              <p className="text-sm text-gray-400 mt-2">Click edit on a content block or add a new one.</p>
            </div>
          )}
        </div>
      </div>
