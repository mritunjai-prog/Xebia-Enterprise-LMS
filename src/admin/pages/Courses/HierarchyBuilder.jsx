    ...
    <div className="fixed inset-0 z-[100] bg-background flex flex-col animate-in slide-in-from-bottom-8 duration-500">
                  ...
                  <div className="flex items-center gap-3" onClick={e => e.stopPropagation()}>
                    <button 
                      onClick={() => {
                        ...
                        setSubmoduleForm({ title: '', description: '', metaTitle: '', metaDescription: '', canonicalUrl: '' });
      ...
      <AnimatePresence>
        {isModuleModalOpen && (
          <div className="fixed inset-0 z-[110] bg-background/50 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-lg glass rounded-2xl p-6 border border-border/40 shadow-2xl">
              ...
              <form onSubmit={handleAddModule} className="space-y-4">
                <div>
                  ...
                  <input required type="text" className="w-full px-4 py-2.5 bg-background border border-border/50 rounded-xl text-sm focus:border-indigo-500 outline-none" value={moduleForm.title} onChange={e => setModuleForm({...moduleForm, title: e.target.value})} />
                </div>
                <div>
                  <label className="text-xs font-bold text-muted-foreground uppercase block mb-1.5">Description</label>
                  <textarea rows={3} className="w-full px-4 py-2.5 bg-background border border-border/50 rounded-xl text-sm focus:border-indigo-500 outline-none resize-none" value={moduleForm.description} onChange={e => setModuleForm({...moduleForm, description: e.target.value})} />
                </div>
                <button type="submit" className="w-full py-3 rounded-xl bg-indigo-500 text-white font-bold hover:bg-indigo-600 transition-colors shadow-lg shadow-indigo-500/20 mt-4">Save Module</button>
              </form>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {isSubmoduleModalOpen && (
          <div className="fixed inset-0 z-[110] bg-background/50 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-xl glass rounded-2xl p-6 border border-border/40 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h2 className="text-xl font-bold font-display text-foreground">Create Submodule</h2>
                  <p className="text-xs text-muted-foreground">Maps to SubmoduleEntity. Contains intense SEO configurations.</p>
                </div>
              ...
              <form onSubmit={handleAddSubmodule} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="text-xs font-bold text-muted-foreground uppercase block mb-1.5">Submodule Topic Title</label>
                    <input required type="text" className="w-full px-4 py-2.5 bg-background border border-border/50 rounded-xl text-sm focus:border-amber-500 outline-none" value={submoduleForm.title} onChange={e => setSubmoduleForm({...submoduleForm, title: e.target.value})} />
                  </div>
                  <div className="col-span-2">
                  ...
                  <div className="col-span-2 border-t border-border/50 pt-4 mt-2">
                    <p className="text-xs font-bold text-amber-500 uppercase tracking-widest mb-4 flex items-center gap-2"><Globe className="w-3.5 h-3.5" /> SEO Metadata Wrapper</p>
                  </div>
                  <div className="col-span-2">
                    <label className="text-xs font-bold text-muted-foreground uppercase block mb-1.5">Canonical URL Slug</label>
                  ...
                  <div>
                    ...
                    <input type="text" className="w-full px-4 py-2.5 bg-background border border-border/50 rounded-xl text-sm focus:border-amber-500 outline-none" value={submoduleForm.metaTitle} onChange={e => setSubmoduleForm({...submoduleForm, metaTitle: e.target.value})} />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-muted-foreground uppercase block mb-1.5">Meta Description</label>
                    <input type="text" className="w-full px-4 py-2.5 bg-background border border-border/50 rounded-xl text-sm focus:border-amber-500 outline-none" value={submoduleForm.metaDescription} onChange={e => setSubmoduleForm({...submoduleForm, metaDescription: e.target.value})} />
                  </div>
                </div>
                <button type="submit" className="w-full py-3 rounded-xl bg-amber-500 text-white font-bold hover:bg-amber-600 transition-colors shadow-lg shadow-amber-500/20 mt-4">Save Submodule Container</button>
              </form>
            </motion.div>
          </div>
        )}
              ...
              <form onSubmit={handleAddContent} className="space-y-4">
                ...
                {(contentForm.type === 'PDF' || contentForm.type === 'CODE') && (
                  <div>
                    ...
                    <input required type="text" className="w-full px-4 py-2.5 bg-background border border-border/50 rounded-xl text-sm focus:border-emerald-500 outline-none" value={contentForm.documentUrl} onChange={e => setContentForm({...contentForm, documentUrl: e.target.value})} />
                  </div>
                )}
