...
function SectionCard({ color = BRAND, title, icon, children }) {
  return (
    <div className="bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-xl overflow-hidden shadow-sm transition-colors">
      <div
        className="flex items-center gap-2.5 px-5 py-3.5 border-b border-gray-100 dark:border-[#2e2e3e]"
...
function Input({ ...props }) {
  return (
    <input
      {...props}
      className={clsx(
        "w-full px-3 py-2 border border-gray-200 dark:border-[#3a3a4a] rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:border-[#6C1D5F] focus:ring-1 focus:ring-[#6C1D5F] bg-white dark:bg-[#1a1a24] transition-all",
        props.className,
      )}
    />
  );
...
function Textarea({ ...props }) {
  return (
    <textarea
      {...props}
      className={clsx(
        "w-full px-3 py-2 border border-gray-200 dark:border-[#3a3a4a] rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:border-[#6C1D5F] focus:ring-1 focus:ring-[#6C1D5F] bg-white dark:bg-[#1a1a24] resize-none transition-all",
        props.className,
      )}
    />
  );
  ...
  return (
    <div className="relative">
      <select
        {...props}
        className={clsx(
          "w-full px-3 py-2 border border-gray-200 dark:border-[#3a3a4a] rounded-lg text-sm text-gray-900 dark:text-white outline-none focus:border-[#6C1D5F] bg-white dark:bg-[#1a1a24] appearance-none transition-all",
          props.className,
        )}
      >
        {children}
      ...
      <svg
        className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 dark:text-gray-500 pointer-events-none"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
    ...
    <div className="flex items-start gap-3">
      <button
        ...
        className={clsx(
          ...
          checked ? "bg-[#01AC9F]" : "bg-gray-200 dark:bg-[#3a3a4a]",
        )}
      >
        <span
          className={clsx(
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-2">
          <span className="w-5 h-5 rounded-full bg-gray-100 dark:bg-[#252535] text-gray-500 dark:text-gray-400 text-[11px] font-bold flex items-center justify-center shrink-0">
            {i + 1}
          </span>
          <Input
            value={item}
          ...
          <button
            type="button"
            onClick={() => remove(i)}
            className="p-1 text-gray-400 dark:text-gray-500 hover:text-destructive dark:hover:text-destructive transition-colors shrink-0"
          >
      ...
      <button
        type="button"
        onClick={add}
        className="flex items-center gap-1.5 text-[12px] font-semibold px-3 py-1.5 rounded-lg border border-dashed border-gray-300 dark:border-[#3a3a4a] text-gray-500 dark:text-gray-400 hover:border-[#6C1D5F] hover:text-[#6C1D5F] transition-colors"
      >
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <Field
              ...
              required
              hint="A short, catchy tagline (max 200 chars)."
            />
            <button
              type="button"
              onClick={() =>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="h-28 bg-gray-50 dark:bg-[#1a1a24] border border-dashed border-gray-200 dark:border-[#3a3a4a] rounded-xl flex items-center justify-center overflow-hidden">
              {form.thumbnailImageUrl ? (
                ...
                <div className="text-center text-gray-300 dark:text-gray-600">
                  ...
                  <div className="text-xs">Thumbnail Preview</div>
                </div>
              )}
            </div>
            <div className="h-28 bg-gray-50 dark:bg-[#1a1a24] border border-dashed border-gray-200 dark:border-[#3a3a4a] rounded-xl flex items-center justify-center">
              {form.promoImageUrl ? (
                ...
                <div className="text-center text-gray-300 dark:text-gray-600">
                  <div className="text-3xl mb-1">🖼️</div>
                  <div className="text-xs">OG Image Preview</div>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-5">
          <Field label="Learning Outcomes" hint="What students will learn">
            <div className="flex items-center justify-end mb-2">
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {generatingField === "learningOutcomes" ? (
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                ) : (
                  <Wand2 className="w-3.5 h-3.5" />
                )}
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed mb-2"
            >
              {generatingField === "careerOpportunities" ? (
                <Loader2 className="w-3.5 h-3.5 animate-spin" />
              ) : (
                <Wand2 className="w-3.5 h-3.5" />
              )}
...
function SeoMetaStep({ form, setForm }) {
  ...
  const scoreColor =
    seoScore >= 70 ? "text-[#01AC9F]" : seoScore >= 40 ? "text-destructive" : "text-destructive";
          ...
          <FormRow>
            <Field label="focusKeyphrase (Google/SEO)">
              <Input
                ...
                onChange={(e) => setForm((f) => ({ ...f, focusKeyphrase: e.target.value }))}
            ...
            <Field label="focusKeyword (SEMrush)">
              <Input
                ...
                placeholder="spring boot course"
              />
            </Field>
          </FormRow>
          <FormRow>
            <Field label="socialMediaTitle (Linked label)">
              <Input
                value={form.socialMediaTitle}
                onChange={(e) => setForm((f) => ({ ...f, socialMediaTitle: e.target.value }))}
                placeholder="Spring Boot Masterclass"
              />
            </Field>
            <Field label="searchEngineTitle (Google SEO)">
              <Input
                value={form.searchEngineTitle}
                onChange={(e) => setForm((f) => ({ ...f, searchEngineTitle: e.target.value }))}
                placeholder="Spring Boot Masterclass | Xebia LMS"
              />
            </Field>
          </FormRow>
          <FormRow>
            <Field label="courseDescription (JSON-LD)">
              <Textarea
                value={form.courseDescriptionSeo}
                onChange={(e) => setForm((f) => ({ ...f, courseDescriptionSeo: e.target.value }))}
                rows={2}
                placeholder="Same or shorter version of your course description..."
              />
            </Field>
            <Field label="courseDescription (Twitter)">
              <Textarea
                value={form.courseDescriptionTwitter}
                onChange={(e) =>
                  setForm((f) => ({ ...f, courseDescriptionTwitter: e.target.value }))
                }
                rows={2}
                placeholder="Tweet-sized description..."
              />
            </Field>
          </FormRow>
        </div>
        ...
        <div className="space-y-4">
          <FormRow>
            ...
            <Field label="ogType">
              ...
              >
                {["website", "article", "course", "video.movie"].map((t) => (
          ...
          <Field label="ogDescription (*)">
            <Textarea
              ...
              rows={2}
              placeholder="OG description for Facebook, LinkedIn..."
            />
          </Field>
          <FormRow>
        ...
        <div className="space-y-4">
          ...
          <Field label="twitterDescription (*)">
            <Textarea
              ...
              rows={2}
              placeholder="Short Twitter description..."
            />
          </Field>
        ...
        <div className="space-y-4">
          <Field label="courseCode">
            <Input
              ...
              placeholder="spring-boot-masterclass-001"
            />
          </Field>
          <div className="space-y-2">
            {[
              ...
              {
                label: "faqSchema (JSON-LD)",
                key: "faqSchema",
                placeholder: '{"@type":"FAQPage","mainEntity":[...]}',
              },
              {
                label: "reviewSchema (JSON-LD)",
                key: "reviewSchema",
                placeholder: '{"@type":"AggregateRating",...}',
              },
        ...
        <div className="space-y-4">
          <FormRow>
            ...
            <Field label="LabelCard (alias / label)">
              <Select
                value={form.labelCard}
                onChange={(e) => setForm((f) => ({ ...f, labelCard: e.target.value }))}
              >
                {["None", "New", "Bestseller", "Popular", "Updated", "Free"].map((l) => (
                  <option key={l}>{l}</option>
                ))}
              </Select>
            </Field>
          ...
          <Field label="Note Description">
            <Input
              ...
              onChange={(e) => setForm((f) => ({ ...f, noteDescription: e.target.value }))}
              placeholder="To the landing description"
            />
          </Field>
          <Field label="Badge Image (***)" hint="Enter Badge Image URL">
            <Input
              value={form.badgeImage}
              onChange={(e) => setForm((f) => ({ ...f, badgeImage: e.target.value }))}
              placeholder="https://..."
            />
          </Field>
          <Field label="Build Image (***)" hint="Enter Build Image URL">
            <Input
              value={form.buildImage}
              onChange={(e) => setForm((f) => ({ ...f, buildImage: e.target.value }))}
              placeholder="https://..."
            />
          </Field>
        </div>
      ...
      <SectionCard color="#ec4899" title="Configurations (A/B testing | Advanced)" icon="🧪">
        <div className="space-y-4">
          ...
          <Field label="legalText (***) · Site-wide legal override">
            <Textarea
              value={form.legalTextOverride}
              onChange={(e) => setForm((f) => ({ ...f, legalTextOverride: e.target.value }))}
              rows={2}
              placeholder="Optional site-wide legal override..."
            />
          </Field>
        </div>
      </SectionCard>
...
export default function CreateCourse({ editData, categories = [], onBack, onSaved }) {
  ...
  const handleSave = async (asDraft = false) => {
    ...
    try {
      ...
      const payload = {
        ...
        metaKeywords:
          typeof form.metaKeywords === "string"
            ? form.metaKeywords
                .split(",")
                .map((s) => s.trim())
                .filter(Boolean)
  ...
  const seoScore = Math.min(
    100,
    [form.seoTitle, form.seoDescription, form.ogTitle, form.metaKeywords].filter(Boolean).length *
      18,
  );
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {currentStep === 0
                  ...
                  : "Set up this page to rank higher and optimise SEO titles."}
              </p>
            </div>
          </div>
              ...
              <div className="rounded-xl overflow-hidden border border-gray-100 dark:border-[#2e2e3e]">
                <div className="h-24 bg-gray-100 dark:bg-[#1a1a24] overflow-hidden flex items-center justify-center relative">
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 z-0">
                    <span className="text-3xl font-bold opacity-30 uppercase">
                      {form.title ? form.title.substring(0, 2) : "CO"}
                    </span>
                  </div>
                  {form.thumbnail && (
                    <img
                ...
                <div className="p-3 space-y-2 bg-white dark:bg-[#1a1a24]">
                  <div className="flex gap-1.5 flex-wrap">
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-destructive/10 dark:bg-destructive/20 text-destructive dark:text-destructive">
                      {form.level}
                    </span>
                  </div>
                  <p className="text-sm font-bold text-gray-900 dark:text-white line-clamp-2">
                    {form.title || "Spring Boot Masterclass"}
                  </p>
                  <p className="text-[11px] text-gray-400 dark:text-gray-500 line-clamp-2">
                    {form.shortDescription || "Course description preview..."}
                  </p>
                  <div className="flex items-center gap-2 text-[10px] text-gray-500 dark:text-gray-400">
                    ...
                    <span className="flex items-center gap-0.5">
                      ...
                      {form.durationHours || "0"} hrs
                    </span>
                    <span className="flex items-center gap-0.5">
                      <BookOpen className="w-2.5 h-2.5" />0 Modules
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-[#15151f] border border-gray-200 dark:border-[#2e2e3e] rounded-xl p-4 shadow-sm transition-colors">
            ...
            <div className="space-y-2">
              {[
                ...
                {
                  label: "Duration",
                  val: form.durationHours ? `${form.durationHours} hrs` : "—",
                  ok: !!form.durationHours,
                },
                {
                  label: "Description",
                  val: form.description ? "Filled" : "Empty",
                  ok: !!form.description,
                },
                ...
                <div key={label} className="flex justify-between items-center text-[12px]">
                  ...
                  <span
                    className={clsx(
                      ...
                      ok ? "text-gray-800 dark:text-gray-200" : "text-destructive",
                    )}
                  >
                    {val ? (val.length > 16 ? val.slice(0, 14) + "..." : val) : "—"}
                    {ok ? (
                      <Check className="w-3 h-3 text-[#01AC9F]" />
                    ) : (
                ...
                <div className="pt-2 border-t border-gray-100 dark:border-[#2e2e3e]">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={clsx(
                        "text-sm font-black",
                        seoScore >= 70 ? "text-[#01AC9F]" : "text-destructive",
                      )}
                    >
                      {seoScore}
                    </span>
          ...
          <div className="bg-white dark:bg-[#15151f] rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                ...
                with the same Category and Level already exists. Please make it unique.
              </p>
              <button
