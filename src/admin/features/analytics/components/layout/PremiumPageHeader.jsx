...
export function PremiumPageHeader({ title, description, icon: Icon, badgeText, badgeColor = 'indigo' }) {
  ...
  return (
    <div className="relative overflow-hidden mb-10 p-8 sm:p-10 bg-white/70 dark:bg-[#15151f]/70 rounded-3xl border border-white/50 dark:border-[#2e2e3e] shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl group">
      ...
      <div className={clsx("absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-bl rounded-full blur-[100px] -translate-y-1/2 translate-x-1/3 opacity-50 dark:opacity-70 pointer-events-none transition-all duration-700 group-hover:scale-110 group-hover:opacity-100", theme.glow)} />
      ...
      <div className="relative z-10 flex flex-col sm:flex-row sm:items-start justify-between gap-6">
        <div className="flex items-start gap-5">
          {Icon && (
            <div className={clsx("w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 border border-white/20 dark:border-white/5 shadow-inner backdrop-blur-md transition-transform duration-500 group-hover:scale-105", theme.iconBg, theme.text)}>
          ...
          <div>
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              ...
              {badgeText && (
                <Badge className={clsx("px-3 py-1 rounded-full font-bold shadow-sm backdrop-blur-md border", theme.bg, theme.text, theme.border)}>
                  {badgeText}
                </Badge>
              )}
            </div>
