...
export function DonutChart({ title, description, data, dataKey = "value", nameKey = "name", className }) {
  ...
  const defaultColors = ['#5C4F61', '#855889', '#533754', '#91759E', '#4A1E47', '#B8AFCF', '#D3CCEC', '#6C1D5F'];
        ...
        <div className="h-[280px] w-full relative">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
              ...
              >
                {data.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={`url(#colorUv-${index})`} className="hover:opacity-80 transition-opacity outline-none cursor-pointer" />
                ))}
              </Pie>
              <Tooltip 
                cursor={false}
