      ...
      <aside className={clsx("sidebar sidebar-admin bg-white dark:bg-[#15151f]", isSidebarCollapsed && "collapsed")}>
        ...
        <div className="sidebar-nav custom-scrollbar" style={{ overflowY: 'auto' }}>
          ...
          <NavItem
            icon={Tag}
            label="Categories"
            isActive={currentPath === '/admin/categories' || currentPath === '/admin/categories/' || currentPath.startsWith('/admin/categories/')}
            onClick={() => handleNavClick('/admin/categories')}
          />
          <NavItem
            icon={BookOpen}
            label="Courses"
            isActive={currentPath === '/admin/courses' || currentPath === '/admin/courses/' || currentPath.startsWith('/admin/courses/')}
            onClick={() => handleNavClick('/admin/courses')}
          />
          <NavItem
            icon={Layers}
            label="Curriculum"
            isActive={currentPath === '/admin/curriculum' || currentPath.startsWith('/admin/curriculum')}
            onClick={() => handleNavClick('/admin/curriculum')}
          />
          <NavItem
            icon={LogOut}
